...
import { getInfoDoctor } from '../../services/doctorService';
...
function InfoDoctor() {
    ...
    const handleGetInfoDoctor = async (id) => {
        try {
            const res = await getInfoDoctor(id);
            console.log('info doctor', res);
        } catch (error) {
            console.log(error);
        }
    };
                    ...
                    <Col span={18}>
                        <Row>
                            <Col md={12} style={{ padding: '30px' }}>
                                ...
                                <Space direction="vertical" className={cx('info-contact')}>
                                    <Button className={cx('btn-book')}>Book an Appointment</Button>
                                    <Button className={cx('btn-view')}>View Timetable</Button>
                                </Space>
                            </Col>
                            <Col md={12} style={{ padding: '30px' }}>
                                <Typography.Paragraph className={cx('text')}>
                                    Etiam sapien sem magna at vitae pulvinar congue augue egestas pretium neque id
                                    viverra suscipit egestas magna porta ratione, mollis risus lectus porta rutrum arcu
                                    aenean primis in augue luctus neque purus ipsum neque dolor primis suscipit in magna
                                    dignissim, porttitor hendrerit diam
                                </Typography.Paragraph>
                                <Typography.Paragraph className={cx('text')}>
                                    In at mauris vel nisl convallis porta at vitae dui. Nam lacus ligula, vulputate
                                    molestie bibendum quis, aliquet elementum massa. Vestibulum ut sagittis odio. Ac
                                    massa lorem. Fusce eu cursus est. Fusce non nulla vitae massa placerat vulputate vel
                                ...
                                <Typography.Paragraph className={cx('text')}>
                                    ...
                                    etiam sapien sem sagittis congue tempor gravida
                                </Typography.Paragraph>
                                <List
                                    dataSource={data}
                                    renderItem={(item) => (
